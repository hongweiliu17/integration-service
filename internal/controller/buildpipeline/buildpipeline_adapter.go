...
// to prevent PR/MR from being automerged unexpectedly
func (a *Adapter) EnsureIntegrationTestReportedToGitHub() (controller.OperationResult, error) {
	...
	if metadata.HasAnnotation(a.pipelineRun, tekton.SnapshotNameLabel) {
		...
		if err := tekton.AnnotateBuildPipelineRun(a.context, a.pipelineRun, h.SnapshotCreationReportAnnotation, "SnapshotCreated", a.client); err != nil {
			a.logger.Error(err, "failed to set annotation build pipelineRun")
			return controller.RequeueWithError(err)
		}
		return controller.ContinueProcessing()
	}
	...
	if allIntegrationTestScenarios != nil {
		// we need to handle context in integrationTestScenario, but ignore `group` right now defer the `group` context
		...
		if len(*integrationTestScenarios) == 0 {
			a.logger.Info("no need to report integration test status since no integrationTestScenario can be applied to build pipelinerun")
			return controller.ContinueProcessing()
		}

		err := a.ReportIntegrationTestStatusAccordingToBuildPLR(a.pipelineRun, integrationTestScenarios, integrationTestStatus)
		if err != nil {
		...
		if err = tekton.AnnotateBuildPipelineRun(a.context, a.pipelineRun, h.SnapshotCreationReportAnnotation, integrationTestStatus.String(), a.client); err != nil {
			a.logger.Error(err, fmt.Sprintf("failed to write build plr metadata annotation %s", h.SnapshotCreationReportAnnotation))
...
func (a *Adapter) ReportIntegrationTestStatusAccordingToBuildPLR(pipelineRun *tektonv1.PipelineRun, integrationTestScenarios *[]v1beta2.IntegrationTestScenario, integrationTestStatus intgteststat.IntegrationTestStatus) error {
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		err := a.iterateIntegrationTestInStatusReport(reporter, a.pipelineRun, integrationTestScenarios, integrationTestStatus)
		if err != nil {
...
// iterates integrationTestScenarios to report to destination snapshot for them
func (a *Adapter) iterateIntegrationTestInStatusReport(reporter status.ReporterInterface,
	...
	intgteststatus intgteststat.IntegrationTestStatus) error {

	detail := generationDetails(buildPLR, intgteststatus)
	...
	for _, integrationTestScenario := range *integrationTestScenarios {
		...
		testReport, reportErr := status.GenerateTestReport(a.context, a.client, integrationTestStatusDetail, buildPLR, a.component.Name)
		if reportErr != nil {
		...
		if reportStatusErr := reporter.ReportStatus(a.context, *testReport); reportStatusErr != nil {
			return fmt.Errorf("failed to report status AND write snapshot report status metadata: %w", reportStatusErr)
		}
	}
	return nil
}
...
func getSnapshotCreationStatusFromBuildPLR(plr *tektonv1.PipelineRun) intgteststat.IntegrationTestStatus {
	...
	// when build pipeline is triggered/retriggered, we need to set integreation test to pending
	if !h.HasPipelineRunFinished(plr) && !metadata.HasAnnotationWithValue(plr, h.SnapshotCreationReportAnnotation, tekton.BuildPLRInProgress) {
	...
	if h.HasPipelineRunFinished(plr) && !h.HasPipelineRunSucceeded(plr) && metadata.HasAnnotationWithValue(plr, h.SnapshotCreationReportAnnotation, tekton.BuildPLRFailed) {
	...
	if h.HasPipelineRunSucceeded(plr) && metadata.HasAnnotationWithValue(plr, h.CreateSnapshotAnnotationName, "Failed to create snapshot") && !metadata.HasAnnotationWithValue(plr, h.SnapshotCreationReportAnnotation, tekton.SnapshotCreationFailed) {
		integrationTestStatus = intgteststat.SnapshotCreationFailed
	}
	return integrationTestStatus
}

func generationDetails(buildPLR *tektonv1.PipelineRun, integrationTestStatus intgteststat.IntegrationTestStatus) string {
	details := ""
	...
	if integrationTestStatus == intgteststat.SnapshotCreationFailed {
		if failureReason, ok := buildPLR.Annotations[h.CreateSnapshotAnnotationName]; ok {
			details = fmt.Sprintf("build Pipelinerun %s/%s succeeds but snapshot is not created: %s", buildPLR.Namespace, buildPLR.Name, failureReason)
		}
		details = "failed to create snapshot but can't find reason from build plr annotatation"
	}
